#!/usr/bin/env python3
# mini pi에서 /opt/ros/noetic/lib/joy_teleop/joy_teleop.py에 위치
import importlib
...
class JoyTeleop:
    ...
    def register_service(self, name, command):
        ...
        try:
            ...
            self.srv_clients[service_name] = self.AsyncServiceProxy(service_name, service_type, service_persistent)
    ...
    def run_command(self, command, joy_state):
        ...
        if cmd["type"] == "topic":
            ...
            if cmd["action_name"] in self.offline_actions:
                rospy.logerr("command {} was not played because the action " "server was unavailable. Trying to reconnect...".format(cmd["action_name"]))
            ...
            if cmd["service_name"] in self.offline_services:
                rospy.logerr("command {} was not played because the service " "server was unavailable. Trying to reconnect...".format(cmd["service_name"]))
            ...
            for mapping in cmd["axis_mappings"]:
                if "axis" in mapping:
                    ...
                    if len(joy_state.buttons) > mapping["button"]:
                        ...
                        rospy.logerr(
                            "Joystick has only {} buttons (indexed from 0), but #{} was referenced in config.".format(len(joy_state.buttons), mapping["button"])
                        )
                        val = 0.0
                else:
