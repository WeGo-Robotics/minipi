...
def parse_args():
    ...
    p.add_argument("--yolo-topic", default="/yolo/detections")
...
class Viz:
    def __init__(self, args):
        ...
        rospy.loginfo("  out(raw)=%s  out(comp)=%s  pub_fps=%.1f  jpeg=%d", args.out_topic, args.out_topic_comp, args.pub_fps, args.jpeg_quality)
    ...
    def _handle_frame(self, frame, header):
        ...
        if current_display_txt and current_display_txt != "avoid wall":
            cv2.putText(frame, current_display_txt, (8, 35), cv2.FONT_HERSHEY_COMPLEX, 1.3, (255, 255, 255), 2, cv2.LINE_AA)
        ...
        if self.pi_mode_text == "avoid wall" and not is_mode_old:
            ...
            cv2.putText(frame, text_to_display, (text_x, text_y), font, font_scale, text_color, font_thickness, cv2.LINE_AA)
