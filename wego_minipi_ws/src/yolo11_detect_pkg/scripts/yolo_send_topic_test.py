...
os.environ.pop("ROS_LOG_CONFIG_FILE", None)  # 추가: ROS 기본 로깅 설정도 제거
import numpy as np
import cv2
import rospy
from std_msgs.msg import Header
from sensor_msgs.msg import Image
from sim2real_msg.msg import Yolo, YoloDetect
from rknnlite.api import RKNNLite

# ===== 경로/하이퍼파라미터 =====
RKNN_PATH = "/home/hightorque/wego_minipi_ws/src/yolo11_detect_pkg/config/rknn/1020_best-rk3588.rknn"
YAML_PATH = "/home/hightorque/wego_minipi_ws/src/yolo11_detect_pkg/config/rknn/1020_metadata.yaml"  # 선택(없어도 OK)
IMG_SIZE = (640, 640)  # (W, H) — 모델 입력 크기와 동일해야 함
CONF_THRES = 0.25
IOU_THRES = 0.45

# 기본 파라미터(rosparam으로 덮어쓰기 가능)
DEFAULT_IMAGE_ONLY = True
DEFAULT_IMAGE_PATH = "/home/hightorque/wego_minipi_ws/src/yolo11_detect_pkg/scripts/test.png"
DEFAULT_PUB_RATE_HZ = 10.0
DEFAULT_IMAGE_TOPIC = "/usb_cam/image_raw"


# -------------------- 유틸 --------------------
def letterbox(bgr, new_shape=(640, 640), color=(114, 114, 114)):
    """비율 유지 + 패딩해서 new_shape(H,W)로 맞추기"""
    h0, w0 = bgr.shape[:2]
    ...
    return canvas, (x0, y0, r, (w0, h0))  # pad_x, pad_y, scale, (orig_w, orig_h)
...
class YoloInfo(object):
    def __init__(self):
        ...
        rospy.init_node("yolo_info_node", anonymous=False, log_level=rospy.INFO, disable_rosout=True)
        # params
        self.image_only = bool(rospy.get_param("~image_only", DEFAULT_IMAGE_ONLY))
        self.image_path = str(rospy.get_param("~image_path", DEFAULT_IMAGE_PATH))
        self.pub_rate_hz = float(rospy.get_param("~pub_rate", DEFAULT_PUB_RATE_HZ))
        ...
        self._printed_shapes = False  # 출력 텐서 shape 1회 로그용
    ...
    def _as_NC(self, out0):
        """RKNN out을 (N,C)로 정규화. (1,5,N)/(1,N,5)/(5,N)/(N,5) 등 처리"""
        a = np.array(out0)
        if a.ndim == 3:
            # (1,5,N) → (1,N,5)
    ...
    def postprocess(self, outputs, meta):
        ...
        if C == 5:
            ...
            for (xc, yc, wv, hv), sc in zip(xywh, conf):
                ...
                dets.append(dict(cls=0, conf=float(sc), x_center=ox, y_center=oy, w=ow, h=oh))
            return dets

        # ---- YOLOv5/8류: [xc,yc,w,h,obj, p1..pNc] ----
        if C > 6:
            xywh = a4
            if is_norm:
                xywh[:, 0] *= W
            ...
            for i in range(N):
                if scores[i] < CONF_THRES or xywh[i, 2] <= 1 or xywh[i, 3] <= 1:
                    continue
                xc, yc, wv, hv = xywh[i]
                ox, oy, ow, oh = to_orig_xywh(xc, yc, wv, hv)
                dets.append(dict(cls=int(cls_ids[i]), conf=float(scores[i]), x_center=ox, y_center=oy, w=ow, h=oh))
        ...
        if x1y1x2y2_like:
            ...
            for i in range(N):
                if scores[i] < CONF_THRES or w[i] <= 1 or h[i] <= 1:
                    continue
    ...
    def _infer_and_publish(self, frame, header=None):
        ...
        out = Yolo()
        out.header = header if header is not None else Header(stamp=rospy.Time.now(), frame_id="test_image")
        ...
        # 사용자의 Yolo.msg에 count 필드가 있을 수도 있어서 안전하게 시도
        try:
    ...
    def loop_image(self):
        ...
        if img is None:
            print("[ERR ] Failed to read image:", self.image_path)
            return
        print(f"[INFO] Start image loop: {self.image_path} @ {self.pub_rate_hz} Hz")
        rate = rospy.Rate(self.pub_rate_hz)
        while not rospy.is_shutdown():
            self._infer_and_publish(img)
            rate.sleep()
...
def main():
    node = YoloInfo()
    if node.image_only:
        node.loop_image()  # 이미지 한 장을 계속 퍼블리시
    else:
        print(f"[INFO] Live mode. Subscribing: {node.image_topic}")
        rospy.spin()
