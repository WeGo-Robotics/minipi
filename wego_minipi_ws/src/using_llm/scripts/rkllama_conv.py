...
from sim2real_msg.msg import Joy  # sim2real_msg 토픽에 대한 가정
from geometry_msgs.msg import Twist
import numpy as np
from math import pi

RK_BASE = os.environ.get("RK_BASE", "http://127.0.0.1:8080")
MODEL = os.environ.get("RK_MODEL", "your_rk_model")  # 자신이 다운받은 모델의 이름으로 설정
...
class RkllamaPi:
    def __init__(self):
        self.TOOLS = [
            ...
            },
            {
                ...
                "function": {
                    ...
                    "parameters": {
                        "type": "object",
                        "properties": {"object_name": {"type": "string", "description": "이동할 대상 물체의 이름 (예: 'cup', 'table', 'person')"}},
        ...
        self.joy_dance_pub = rospy.Publisher("/joy_msg", Joy, queue_size=10)
        self.joy_msg = Joy()
    ...
    def usr_input_cb(self, msg: String):
        ...
        messages = [{"role": "system", "content": base_system_prompt}, {"role": "user", "content": user_input}]
    ...
    def chat_once(self, messages, tools):
        ...
        payload = {"model": MODEL, "messages": messages, "tools": tools, "stream": False}  # JSONDecodeError 방지를 위해 스트리밍 비활성화
