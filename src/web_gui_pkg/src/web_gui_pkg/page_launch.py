...
def main_page():
    ...
    with ui.header().classes("bg-slate-900 shadow-lg"):
        with ui.row().classes("w-full items-center h-full max-w-screen-xl mx-auto px-4"):
            ...
            ui.space() # 빈 공간 채우기 (우측 정렬용)

            # [수정됨] CONTROL 페이지 이동 버튼
            # 최신 NiceGUI 버전에서는 ui.open 대신 ui.navigate.to 사용
            ui.button("CONTROL MODE", icon="gamepad", on_click=lambda: ui.navigate.to('/control')) \
                .props("flat color=white") \
                .classes("font-bold mr-4 border border-white/30 hover:bg-white/10")
