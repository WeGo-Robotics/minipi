...
class NiceGUIRos:
    ...
    def setup_ros_client(self):
        ...
        try:
            ...
            if hasattr(self.ros_client, 'on_close'):
                ...
                try: self.ros_client.on('close', self.on_close)
                except: pass
        ...
        for pkg in packages:
            ...
            try:
                ...
                if files:
                    ...
                    file_data = {}
                    for f in files:
                        if f in existing_files: file_data[f] = existing_files[f]
                        else: file_data[f] = {"status": "STOPPED", "process": None, "button": None}
    ...
    def get_available_video_devices(self) -> List[str]:
        try: return [d for d in sorted(glob.glob("/dev/video*")) if d[-1].isdigit()]
        except: return ['/dev/video0']

    # ============================================================
    # Launch Control
    # ============================================================
    def toggle_launch(self, pkg: str, file_name: str):
        if pkg not in self.launch_files_data or file_name not in self.launch_files_data[pkg]["files"]: return
    ...
    def stop_all_launches(self):
        for pkg, pkg_data in self.launch_files_data.items():
            for file_name, info in pkg_data["files"].items():
                if info["status"] == "RUNNING": self.stop_launch(pkg, file_name, info)
        self.render_launch_list() 
        ...
        while proc and proc.poll() is None:
            try:
                ...
                if updated and self.log_container:
                    if len(self.log_buffer) > MAX_LOG_LINES: self.log_buffer = self.log_buffer[-MAX_LOG_LINES:]
                    self.log_container.set_text("\n".join(self.log_buffer))
                    try:
                        self.log_container.client.run_javascript(f'document.getElementById("{self.log_container.id}").parentElement.scrollTop = document.getElementById("{self.log_container.id}").parentElement.scrollHeight')
                    except: pass
    ...
    def render_launch_list(self):
        ...
        try:
            ...
            with self.launch_list_container:
                if self.search_status_label: self.search_status_label.visible = False
                ...
                with ui.column().classes("w-full max-w-5xl mx-auto space-y-3"):
                    for pkg, pkg_data in sorted(self.launch_files_data.items()):
                        ...
                        with ui.expansion(pkg, icon="folder", value=has_running).classes("w-full bg-white shadow-sm border border-slate-200 rounded-xl px-2 py-1").props('header-class="text-lg font-bold text-slate-800"'):
    ...
    def render_usb_cam_controller(self, pkg, file_name, devices):
        ...
        with ui.card().classes("w-full shadow-lg border border-indigo-200"):
            ...
            btn = ui.button("RUNNING" if info["status"] == "RUNNING" else "RUN", icon="pause" if info["status"] == "RUNNING" else "play_arrow", color="red-6" if info["status"] == "RUNNING" else "green-6").classes('w-full mt-4')
            info["button"] = btn
    ...
    def _to_bytes(data):
        ...
        if isinstance(data, str):
            ...
            except: return data.encode("latin1")
        return str(data).encode("latin1")

    def subscribe_image_topic(self, topic):
        self.unsubscribe_current_image_topic()
        if not topic or not self.ros_client or not self.ros_client.is_connected: return
        ...
        def cb(msg):
            ...
            try:
                ...
                if not data: return
                jb = None
                if is_comp: jb = self._to_bytes(data)
                else:
                    ...
                    if c==3 and msg.get("encoding")=="rgb8": img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
                    ok, enc = cv2.imencode(".jpg", img)
                    if ok: jb = enc.tobytes()
                if jb: 
        ...
        if self.image_display: self.image_display.set_source("/video_feed")
    ...
    def unsubscribe_current_image_topic(self):
        ...
        if self.last_frame_label: self.last_frame_label.set_text("Last frame: -")
    ...
    def _render_chat_log(self):
        if not self.chat_log_container: return
        html = ""
    ...
    def _remove_loading_indicator(self):
        if self.is_waiting_for_llm:
            if self.chat_log_buffer and self.chat_log_buffer[-1]["sender"] == "Loading": self.chat_log_buffer.pop()
            self.is_waiting_for_llm = False
            self._render_chat_log()

    async def on_send_command(self):
        if not self.chat_input or not self.llm_pub: return
