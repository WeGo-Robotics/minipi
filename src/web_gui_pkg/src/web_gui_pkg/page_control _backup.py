                ...
                with ui.column().classes('w-full md:w-[350px] gap-6'):
                    ...
                    with ui.card().classes('w-full bg-[#2d2d2d] p-6 items-center text-center rounded-lg shadow-lg'):
                        ...
                        with ui.grid(columns=3).classes('gap-3'):
                            ...
                            ui.label('')
                            # [수정됨] .on() 메서드로 이벤트 연결
                            ui.button(icon='arrow_upward').classes(btn_style) \
                                .on('mousedown', lambda: self.publish_vel(0.2, 0.0)) \
                                .on('mouseup', self.stop_vel) \
                                .on('mouseleave', self.stop_vel) \
                                .on('touchstart', lambda: self.publish_vel(0.2, 0.0)) \
                                .on('touchend', self.stop_vel)
                            ui.label('')
                            
                            # Row 2
                            ui.button(icon='arrow_back').classes(btn_style) \
                                .on('mousedown', lambda: self.publish_vel(0.0, 0.5)) \
                                .on('mouseup', self.stop_vel) \
                                .on('mouseleave', self.stop_vel) \
                                .on('touchstart', lambda: self.publish_vel(0.0, 0.5)) \
                                .on('touchend', self.stop_vel)

                            ui.button(icon='stop', on_click=self.stop_vel).classes(stop_style)

                            ui.button(icon='arrow_forward').classes(btn_style) \
                                .on('mousedown', lambda: self.publish_vel(0.0, -0.5)) \
                                .on('mouseup', self.stop_vel) \
                                .on('mouseleave', self.stop_vel) \
                                .on('touchstart', lambda: self.publish_vel(0.0, -0.5)) \
                            ...
                            # Row 3
                            ui.label('')
                            ui.button(icon='arrow_downward').classes(btn_style) \
                                .on('mousedown', lambda: self.publish_vel(-0.2, 0.0)) \
                                .on('mouseup', self.stop_vel) \
                                .on('mouseleave', self.stop_vel) \
                                .on('touchstart', lambda: self.publish_vel(-0.2, 0.0)) \
