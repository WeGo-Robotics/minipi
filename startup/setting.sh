...
# 권한 부여 로직 추가 (TARGET_DESKTOP_DIR1)
if [ -d "$TARGET_DESKTOP_DIR1" ]; then
    echo "$TARGET_DESKTOP_DIR1 디렉토리 내 모든 파일에 실행 권한을 부여합니다."
    # 디렉토리로 이동하여 chmod +x * 실행
    sudo sh -c "cd \"$TARGET_DESKTOP_DIR1\" && chmod +x *"
else
    echo "경고: 대상 디렉토리 $TARGET_DESKTOP_DIR1 를 찾을 수 없습니다."
fi

# 권한 부여 로직 추가 (TARGET_DESKTOP_DIR2)
if [ -d "$TARGET_DESKTOP_DIR2" ]; then
    echo "$TARGET_DESKTOP_DIR2 디렉토리 내 모든 파일에 실행 권한을 부여합니다."
    # 디렉토리로 이동하여 chmod +x * 실행
    sudo sh -c "cd \"$TARGET_DESKTOP_DIR2\" && chmod +x *"
else
    echo "경고: 대상 디렉토리 $TARGET_DESKTOP_DIR2 를 찾을 수 없습니다."
fi
